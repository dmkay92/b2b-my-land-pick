...
export async function generateFilledQuoteTemplate(
  ...
  for (const dayData of draft.itinerary) {
    ...
    if (overnight.type === 'hotel') {
      ...
      const flightRow = scheduleSheet.addRow(['', '', '', '', '기내 숙박', ''])
      flightRow.height = 18
  ...
  for (const cat of CATEGORIES) {
    ...
    if (rows.length === 0) {
      ...
      for (let ri = 0; ri < rows.length; ri++) {
        ...
        row.getCell(4).value = pRow.price
        row.getCell(4).numFmt = '#,##0'
        row.getCell(5).value = pRow.count
        row.getCell(6).value = pRow.quantity
        row.getCell(7).value = { formula: `D${rowIndex}*E${rowIndex}*F${rowIndex}` }
        row.getCell(7).numFmt = '#,##0'
        row.height = 20
        row.eachCell({ includeEmpty: true }, (cell, colNum) => {
