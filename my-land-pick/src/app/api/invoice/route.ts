...
export async function GET(request: NextRequest) {
  ...
  const invoiceType = request.nextUrl.searchParams.get('type') // 'deduction' for deduction invoice
  ...
  // 공제 추가 청구 인보이스
  if (invoiceType === 'deduction') {
    return generateDeductionInvoice(admin, qr, requestId)
  }
...
async function generateDeductionInvoice(admin: any, qr: any, requestId: string) {
  const { data: claims } = await admin
    .from('deduction_claims').select('*')
    .eq('request_id', requestId)
    .eq('status', 'approved')
    .order('created_at', { ascending: true })

  const { data: schedule } = await admin.from('payment_schedules').select('*').eq('request_id', requestId).single()
  const { data: settlement } = await admin.from('quote_settlements').select('landco_quote_total, agency_commission, gmv').eq('request_id', requestId).maybeSingle()
  const { data: installments } = schedule ? await admin.from('payment_installments').select('paid_amount').eq('schedule_id', schedule.id) : { data: [] }

  const paidTotal = (installments ?? []).reduce((s: number, i: { paid_amount: number }) => s + i.paid_amount, 0)
  const totalDeduction = (claims ?? []).reduce((s: number, c: { approved_amount: number | null; total_amount: number }) => s + (c.approved_amount ?? c.total_amount), 0)
  const additionalCharge = Math.max(0, totalDeduction - paidTotal)

  const totalPeople = (qr.adults ?? 0) + (qr.children ?? 0) + (qr.infants ?? 0) + (qr.leaders ?? 0)
  const nights = Math.round((new Date(qr.return_date).getTime() - new Date(qr.depart_date).getTime()) / 86400000)
  const COUNTRY_NAMES: Record<string, string> = { JP: '일본', CN: '중국', VN: '베트남', FR: '프랑스', TH: '태국' }
  const countryName = COUNTRY_NAMES[qr.destination_country] ?? qr.destination_country

  const today = new Date()
  const invNumber = `DED-${today.getFullYear()}${String(today.getMonth() + 1).padStart(2, '0')}${String(today.getDate()).padStart(2, '0')}-${(qr.display_id ?? requestId).replace('REQ-', '')}`

  const claimRows = (claims ?? []).map((c: { items: { name: string; amount: number }[]; approved_amount: number | null; total_amount: number }, idx: number) => {
    const itemNames = c.items.map((i: { name: string }) => i.name).join(', ')
    const amount = c.approved_amount ?? c.total_amount
    return `<tr><td>${idx + 1}</td><td>${itemNames}</td><td>${fmt(amount)}</td></tr>`
  }).join('')
...
<head>
<meta charset="UTF-8">
<title>공제 추가 청구 - ${qr.event_name}</title>
<style>
  @page { size: A4; margin: 0; }
  @media print { body { background: #fff; padding: 0; margin: 0; } .page { box-shadow: none; padding: 52px 56px 48px; width: 100%; } .no-print { display: none !important; } }
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .logo-area .by { font-size: 12px; color: #94a3b8; }
  .header-meta { text-align: right; font-size: 11px; color: #64748b; line-height: 1.5; }
  .invoice-label { font-size: 20px; font-weight: 800; color: #dc2626; text-align: center; letter-spacing: 4px; padding: 12px 0; }
  .parties { margin: 28px 0; }
  .party-section { margin-bottom: 20px; }
  .party-tag { font-size: 11px; font-weight: 700; color: #1e293b; text-decoration: underline; text-underline-offset: 3px; margin-bottom: 8px; }
  .party-name { font-size: 15px; font-weight: 700; color: #1e293b; margin-bottom: 4px; }
  ...
  .event-item .el { color: #94a3b8; width: 60px; flex-shrink: 0; }
  .event-item .ev { color: #334155; font-weight: 600; }
  .items-table { width: 100%; border-collapse: collapse; border: 1px solid #dc2626; }
  .items-table thead th { background: #dc2626; color: #fff; font-size: 11px; font-weight: 600; padding: 10px 16px; text-align: left; }
  .items-table thead th:last-child { text-align: right; }
  .items-table tbody td { font-size: 13px; color: #334155; padding: 13px 16px; border-bottom: 1px solid #fecaca; }
  .items-table tbody td:last-child { text-align: right; font-weight: 600; }
  .summary { margin-top: 20px; border: 1px solid #1e293b; }
  .summary-row { display: flex; justify-content: space-between; padding: 10px 16px; font-size: 12px; border-bottom: 1px solid #e2e8f0; }
  .summary-row:last-child { border-bottom: none; }
  .summary-row .label { color: #64748b; }
  .summary-row .value { color: #334155; font-weight: 600; }
  .summary-row.total { background: #fef2f2; font-size: 14px; }
  .summary-row.total .label { color: #991b1b; font-weight: 700; }
  .summary-row.total .value { color: #dc2626; font-weight: 800; font-size: 16px; }
...
<div class="page">
  <div class="header">
    <div class="header-top">
      ...
      <div class="header-meta">${invNumber}<br>${formatDate(today.toISOString())} 발행</div>
    </div>
    <div class="invoice-label">추가 청구서</div>
  </div>
  ...
  <div class="event-section">
    <div class="event-title">${qr.event_name} (행사 취소)</div>
    <div class="event-grid">
      <div class="event-item"><span class="el">목적지</span><span class="ev">${countryName} ${qr.destination_city}</span></div>
      <div class="event-item"><span class="el">여행기간</span><span class="ev">${qr.depart_date.slice(0, 10)} ~ ${qr.return_date.slice(0, 10)}</span></div>
      <div class="event-item"><span class="el">인원</span><span class="ev">${totalPeople}명</span></div>
  ...
  <table class="items-table">
    <thead><tr><th>No.</th><th>공제 항목</th><th>승인 금액 (원)</th></tr></thead>
    <tbody>${claimRows}</tbody>
  </table>

  <div class="summary">
    <div class="summary-row"><span class="label">결제완료액</span><span class="value">${fmt(paidTotal)}원</span></div>
    <div class="summary-row"><span class="label">실비 공제 (승인 합계)</span><span class="value">-${fmt(totalDeduction)}원</span></div>
    <div class="summary-row total"><span class="label">추가 청구 금액</span><span class="value">${fmt(additionalCharge)}원</span></div>
  </div>

  <div class="bank-section">
    <div class="bank-title">입금 안내</div>
    <div class="bank-grid">
      <div class="bank-row"><span class="rl">은행</span><span class="rv">우리은행</span></div>
      <div class="bank-row"><span class="rl">계좌번호</span><span class="rv">1005-604-520904</span></div>
      <div class="bank-row"><span class="rl">예금주</span><span class="rv">주식회사 마이리얼트립</span></div>
    </div>
  </div>

  <div class="footer">
    <div class="footer-left">주식회사 마이리얼트립 · 209-81-55339<br>서울특별시 서초구 강남대로 311, 18층</div>
