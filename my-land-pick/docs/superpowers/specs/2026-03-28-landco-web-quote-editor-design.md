...
각 Day 하단에 고정으로 표시. 유형 토글로 세 가지 상태 중 선택.
...
**🏨 호텔 선택 시**
- 성급 토글 `[ 3★ | 4★ | 5★ ]` 추가 표시
- 호텔명 입력 필드 표시, 입력 필드 앞에 선택한 성급의 별(⭐) 자동 표시
- Excel 일정표에 `⭐⭐⭐⭐⭐ 호텔명 숙박` 형식으로 자동 입력

**✈️ 기내박 선택 시**
- 성급 토글·입력 필드 숨김, "✈️ 기내 숙박" 텍스트만 표시
- Excel에 `기내 숙박` 으로 자동 입력

**없음 선택 시**
- 행 최소화 (토글만 표시), Excel에 해당 행 미출력
  ...
  "itinerary": [
    {
      ...
      "overnight": { "type": "hotel", "stars": 5, "name": "바르셀로나 W 호텔" }
    }
  ]
}
```

- `type`: `"hotel"` | `"flight"` | `"none"`
- `stars`: 3 | 4 | 5 (type이 `"hotel"`일 때만 사용)
- `name`: 호텔명 (type이 `"hotel"`일 때만 사용)
